...
const defaultCategories = [
  { id: "dev", name: "Daily", icon: "fa-solid fa-code" },
  { id: "social", name: "Media", icon: "fa-solid fa-users" },
  { id: "media", name: "Projects", icon: "fa-solid fa-play" },
  { id: "productivity", name: "Stationery", icon: "fa-solid fa-briefcase" },
];
...
function loadSettings() {
  ...
  return {
    ...
    enabledEngines:
      JSON.parse(localStorage.getItem("enabledEngines")) ??
      defaults.enabledEngines,
    preferredEngine:
      localStorage.getItem("preferredEngine") ?? defaults.preferredEngine,
    weatherLocation:
      localStorage.getItem("weatherLocation") ?? defaults.weatherLocation,
    openWeatherApiKey:
      localStorage.getItem("openWeatherApiKey") ?? defaults.openWeatherApiKey,
    linkBehavior: localStorage.getItem("linkBehavior") ?? defaults.linkBehavior,
    showKeyboardHints:
      localStorage.getItem("showKeyboardHints") ?? defaults.showKeyboardHints,
    ...
    socialLinks:
      JSON.parse(localStorage.getItem("socialLinks")) ?? defaults.socialLinks,
  };
}
...
function updateColorModeVisibility() {
  const colorModeItem = document
    .querySelector('[data-setting="colorMode"]')
...
function renderSearchEngines() {
  ...
  container.innerHTML = settings.enabledEngines
    .map((engineId, index) => {
...
function updateKeyboardHints() {
  ...
  const engineHint =
    engineCount > 1 ? `<kbd>1-${engineCount}</kbd> Engine` : "";
...
function renderSocialLinks() {
  ...
  const visibleLinks = settings.socialLinks.filter(
    (link) => link.visible && link.url,
  );

  if (visibleLinks.length > 0) {
    iconsContainer.innerHTML = visibleLinks
      .map((link) => {
        const target =
          settings.linkBehavior === "new-tab"
            ? "_blank"
            : settings.linkBehavior === "new-window"
              ? "_blank"
              : "_self";
...
function renderLinksGrid() {
  ...
  linksGrid.innerHTML = categories
    .map((category, index) => {
...
function initSettings() {
  ...
  document
    .querySelectorAll("#search-engine-options input")
    .forEach((checkbox) => {
      checkbox.addEventListener("change", () => {
        const enabledEngines = [];
        document
          .querySelectorAll("#search-engine-options input:checked")
          .forEach((cb) => {
...
function populateSettingsUI() {
  ...
  document
    .querySelectorAll("#search-engine-options input")
    .forEach((checkbox) => {
      checkbox.checked = settings.enabledEngines.includes(
        checkbox.dataset.engine,
      );
    });
...
function importSettings(file) {
  ...
  reader.onload = function (e) {
    try {
      ...
      if (
        !confirm(
          "This will replace all your current settings, categories, and links. Continue?",
        )
      ) {
        return;
      }
